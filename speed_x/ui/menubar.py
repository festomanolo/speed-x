...
class SpeedXMenuBarApp(rumps.App):
    def __init__(self):
        super(SpeedXMenuBarApp, self).__init__("Speed-X", quit_button=None)
    ...
    def _build_menu(self):
        ...
        # 1. Header
        self.title_item = rumps.MenuItem("Speed-X: Idle (Ready)")
        self.menu.add(self.title_item)
        self.menu.add(rumps.separator)

        # 2. Command Prompt
        self.ask_item = rumps.MenuItem("Ask Speed-X...", callback=self.on_ask_speed_x)
    ...
    def on_ask_speed_x(self, _):
        """Show native input prompt dialog."""
        window = rumps.Window(
            message="Ask Speed-X in English or Swahili (e.g. 'ongeza sauti', 'play music', 'anza coding'):",
            title="Ask Speed-X",
            default_text="",
            ok="Execute",
            cancel="Cancel",
    ...
    on_ask_laya = on_ask_speed_x

    def on_quit(self, _):
        rumps.quit_application()


LayaMenuBarApp = SpeedXMenuBarApp


def run_menubar():
    app = SpeedXMenuBarApp()
    app.run()
